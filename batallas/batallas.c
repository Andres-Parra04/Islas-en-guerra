...
static void cargarGuerreroAnim(void) {
  ...
  const char *atkL[3] = {"..\\assets\\guerrero\\guerrero_war_move_1_left.bmp",
                         "..\\assets\\guerrero\\guerrero_war_move_2_left.bmp",
                         "..\\assets\\guerrero\\guerrero_war_move_3_left.bmp"};
  const char *atkR[3] = {"..\\assets\\guerrero\\guerrero_war_move_1_right.bmp",
                         "..\\assets\\guerrero\\guerrero_war_move_2_right.bmp",
                         "..\\assets\\guerrero\\guerrero_war_move_3_right.bmp"};
  for (int i = 0; i < 3; i++) {
    if (!gSprGuerrero.attack[0][i])
      gSprGuerrero.attack[0][i] = cargarBmp(atkL[i], NULL, 64, 64);
    if (!gSprGuerrero.attack[1][i])
      gSprGuerrero.attack[1][i] = cargarBmp(atkR[i], NULL, 64, 64);
  }
  const char *dieL[2] = {"..\\assets\\guerrero\\guerrero_war_die_1_left.bmp",
                         "..\\assets\\guerrero\\guerrero_war_die_2_left.bmp"};
  const char *dieR[2] = {"..\\assets\\guerrero\\guerrero_war_die_1_right.bmp",
                         "..\\assets\\guerrero\\guerrero_war_die_2_right.bmp"};
  for (int i = 0; i < 2; i++) {
    if (!gSprGuerrero.death[0][i])
      gSprGuerrero.death[0][i] = cargarBmp(dieL[i], NULL, 64, 64);
    if (!gSprGuerrero.death[1][i])
      gSprGuerrero.death[1][i] = cargarBmp(dieR[i], NULL, 64, 64);
  }
}
...
static void cargarCaballeroAnim(void) {
  ...
  for (int i = 0; i < 3; i++) {
    if (!gSprCaballero.attack[0][i])
      gSprCaballero.attack[0][i] = cargarBmp(atkL[i], NULL, 64, 64);
    if (!gSprCaballero.attack[1][i])
      gSprCaballero.attack[1][i] = cargarBmp(atkR[i], NULL, 64, 64);
  }
  const char *dieL[2] = {"..\\assets\\caballero\\caballero_die_1_left.bmp",
                         "..\\assets\\caballero\\caballero_die_2_left.bmp"};
  const char *dieR[2] = {"..\\assets\\caballero\\caballero_die_1_right.bmp",
                         "..\\assets\\caballero\\caballero_die_2_right.bmp"};
  for (int i = 0; i < 2; i++) {
    if (!gSprCaballero.death[0][i])
      gSprCaballero.death[0][i] = cargarBmp(dieL[i], NULL, 64, 64);
    if (!gSprCaballero.death[1][i])
      gSprCaballero.death[1][i] = cargarBmp(dieR[i], NULL, 64, 64);
  }
}
...
static void statsPorTipo(TipoUnidad t, int *hp, int *danio, int *def,
                         int *crit) {
  switch (t) {
  case TIPO_CABALLERO:
    *hp = 150;
  ...
  case TIPO_GUERRERO:
    ...
    *crit = 10;
    return;
  case TIPO_OBRERO: // Obrero no es tropa de combate, pero lo soportamos con
                    // stats bajos
  default:
    *hp = 60;
    *danio = 8;
...
static void initUnit(BattleUnit *u, Unidad *ref, bool esEnemigo, int hp,
                     int dmg, int def, int crit) {
...
static void resolverTick(void) {
  ...
  for (int a = 0; a < gBatalla.numAliados; a++) {
    ...
    if (dist > BATTLE_RANGE_PIXELS) {
      ...
      if (target->hp <= 0) {
        target->hp = 0;
        target->vivo = false;
        target->animState = ANIM_DEATH;
        target->animFrame = 0;
        target->animFrameMax = 2;
      }
    }
  ...
  for (int a = 0; a < gBatalla.numAliados; a++)
    if (gBatalla.aliados[a].vivo) {
      algunAliado = true;
      break;
    }
  for (int e = 0; e < gBatalla.numEnemigos; e++)
    if (gBatalla.enemigos[e].vivo) {
      algunEnemigo = true;
      break;
    }
...
void batallasRender(HDC hdc, RECT rect, Camara cam) {
  ...
  for (int i = 0; i < gBatalla.numAliados; i++)
    dibujarUnidad(hdc, &gBatalla.aliados[i], cam);
  for (int i = 0; i < gBatalla.numEnemigos; i++)
    dibujarUnidad(hdc, &gBatalla.enemigos[i], cam);
}
