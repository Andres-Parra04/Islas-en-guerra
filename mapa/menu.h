#ifndef MENU_H
#define MENU_H


#define COLOR_NEGRO 0
#define COLOR_AZUL_MARINO 1
#define COLOR_VERDE_OSCURO 2
#define COLOR_ROJO 4
#define COLOR_AZUL_CLARO 9
#define COLOR_BLANCO 7
#define COLOR_AMARILLO 14
#define COLOR_VERDE_CLARO 10
#define COLOR_GRIS 8 // Color para el borde


#define TECLA_ENTER 13
#define TECLA_ESC 27
#define TECLA_ESPECIAL 224 // Prefijo para teclas de función/flecha
#define FLECHA_ARRIBA 72
#define FLECHA_ABAJO 80
#define TECLA_W 'w'
#define TECLA_S 's'


#define ANCHO_CONSOLA 80
#define ALTO_CONSOLA 25

#define INICIO_BORDE_X 21 // Columna donde empieza el marco (centrado en 80)
#define INICIO_BORDE_Y 6  // Fila donde empieza el marco

#define INICIO_TITULO_X 26 // Columna de inicio del título dentro del marco
#define INICIO_MENU_X 27   // Columna de inicio de las opciones



// Oculta el cursor de la consola.
void ocultarCursor();

// Mueve el cursor a la posición (x, y).
void moverCursor(int x, int y);

// Establece los colores de fondo y texto de la consola.
void setColor(int fondo, int texto);



// Dibuja un fondo de mapa estilizado con colores y caracteres ASCII.
void dibujarFondo();

// Muestra la pantalla de Instrucciones del juego.
void mostrarInstrucciones();

// Muestra el menú principal interactivo del juego.
void mostrarMenu();

// Indica la acción seleccionada en el menú principal tras cerrar la ventana.
// 0 = Nueva partida, 1 = Cargar partida
int menuObtenerAccion();

// Isla seleccionada para iniciar la partida (1-3).
int menuObtenerIsla();

// Nombre de la partida seleccionada para cargar (puede ser NULL si es nueva partida).
const char* menuObtenerNombrePartida();

#endif // MENU_H